...
"""
after running the clusterer, we can now properly rank while also considering the cluster labeling
...
MIN_SCORE       = 0.35   # minimum cosine similarity to include an article
MAX_PER_CLUSTER = 2      # max articles per cluster in the briefing
INTEREST_SLOTS  = 5      # articles from interest_vector ranking
LEARNING_SLOTS  = 1      # articles from learning_vector ranking


def fetch_users() -> list[dict]:
    
    #Fetches all users who have both interest and learning vectors embedded.
...
def select_articles(
    ...
    #Scores all articles against a user vector, then selects top articles
    #using cluster-aware deduplication:
    #- Articles must score above MIN_SCORE
    #- Max MAX_PER_CLUSTER articles per cluster
    #- Returns up to n_slots articles sorted by score descending
...
def build_briefing(user: dict, articles: list[dict], article_embeddings: np.ndarray) -> list[str]:
    
    #Runs selection twice — once for interests, once for learning goals —
    #then merges, deduplicates, and returns final ordered article ID list.
    
    interest_vector  = parse_vector(user["interests_raw_vector"])
    learning_vector  = parse_vector(user["learning_goals_raw_vector"])
 
    interest_picks  = select_articles(articles, article_embeddings, interest_vector,  INTEREST_SLOTS)
    learning_picks  = select_articles(articles, article_embeddings, learning_vector,  LEARNING_SLOTS)
 
    # Merge and deduplicate — interest picks come first
    seen_ids = set()
    final = []
 
    for article in interest_picks + learning_picks:
...
def save_briefing(user_id: str, articles: list[dict]):
    
    #Saves the ranked article IDs to the briefings table.
    #Uses upsert on (user_id, date) so re-running doesn't create duplicates.
...
def run():
    ...
    for user in users:
        user_id = user["id"]
        try:
            briefing = build_briefing(user, articles, article_embeddings)
            save_briefing(user_id, briefing)
            ranked += 1
            print(f"  ✓ User {user_id[:8]}... → {len(briefing)} articles selected")
        except Exception as e:
            print(f"  ✗ User {user_id[:8]}... failed: {e}")
            failed += 1

...
Stage 1 of the RSS pipeline — discovers new TechCrunch article URLs
and stores them in the article_links table for Jina to fetch later.

Does NOT fetch full article text — that's the Jina fetcher's job.

Recommended schedule on Railway: every 2 hours
Run manually: python workers/scrapers/rss_techcrunch.py
"""
...
def fetch_rss_entries() -> list[dict]:
    ...
    for entry in feed.entries:
        ...
        if not url or not title:
            continue
        if should_skip(title, "techcrunch"):                                          
            continue
